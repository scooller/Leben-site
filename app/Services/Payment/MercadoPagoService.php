...
class MercadoPagoService implements PaymentGatewayInterface
{
	...
	public function createTransaction(array $data): array
	{
		try {
			...
			FlowLogMatrix::write('payments.mercadopago.create_preference_request', 'MercadoPago: Creando preferencia de pago', [
				...
				'payer_email_present' => filled((string) ($data['payer_email'] ?? '')),
			]);
			...
			throw new \RuntimeException('Error al crear preferencia de pago en MercadoPago: ' . $e->getMessage(), 0, $e);
		}
	}
	...
	protected function getPayment(string $paymentId): array
	{
		try {
			...
			throw new \RuntimeException('Error al obtener información del pago: ' . $e->getMessage(), 0, $e);
		}
	}
	...
	public function refundTransaction(string $transactionId, ?float $amount = null): array
	{
		try {
			...
			if ($response->failed()) {
				throw new \RuntimeException('Error en API de MercadoPago: ' . $response->body());
			}
			...
			throw new \RuntimeException('Error al procesar reembolso: ' . $e->getMessage(), 0, $e);
		}
	}
